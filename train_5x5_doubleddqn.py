...
from double_dqn_sb3 import DoubleDQN  
...
# ----------------------
# Memory logging callback (same as your DQN script)
# ----------------------
...
# ----------------------
# NEW: Exploration metrics callback (same as DQN)
# ----------------------
class ExplorationCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        obs_batch = self.locals.get("new_obs", None)  # (n_envs, C, H, W)
...
model = DoubleDQN(
    ...
    train_freq=4,
    gradient_steps=1,            # replay ratio ≈ 0.25
...
exp_callback = ExplorationCallback(verbose=0)  # <-- NEW
callback = CallbackList([eval_callback, mem_callback, exp_callback])

model.learn(
    total_timesteps=TOTAL_STEPS,
    callback=callback,
    tb_log_name="double_dqn_doorKey5x5",
    progress_bar=True,
)
...
print(f"    tensorboard --logdir results/{ALGO}/{ENV_ID}")
