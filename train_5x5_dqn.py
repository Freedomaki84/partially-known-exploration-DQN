...
# ----------------------
# Function that logs memory
# ----------------------
class PeakMemoryCallback(BaseCallback):
    """
    Logs:
    - Peak RAM/VRAM (end of training)
    - RAM/VRAM time-series (during training)
    - Replay buffer footprint (MB) time-series + peak
    - Model params MB + optimizer state MB (end of training)
    - Memory-normalized metrics (final eval return per GB RAM/VRAM)
    Also writes RUN_ROOT/memory_summary.csv
    """
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.check_interval_steps == 0:
            ...
            # Replay buffer footprint (current + track peak)
    ...
    def _on_training_end(self) -> None:
        ...
        # Final eval mean from EvalCallback (if available)
...
# ----------------------
# Helpers to build envs with the same wrappers
# ----------------------
...
# Evaluation env (separate, identically wrapped, vectorized)
eval_env = DummyVecEnv([lambda: make_wrapped_env(ENV_ID)])
eval_env = VecTransposeImage(eval_env) 
eval_env.seed(SEED + 100)

# ----------------------
# Logging: CSV + TensorBoard + stdout
# ----------------------
logger = configure(RUN_ROOT, ["stdout", "csv", "tensorboard"])


# ----------------------
# Evaluation callback (headless, deterministic)
# ----------------------
...
# ----------------------
# Model (CNN policy for image obs)
# ----------------------
model = DQN(
    "CnnPolicy",
    train_env,
...
callback = CallbackList([eval_callback, mem_callback])
model.learn(
